import LaporanBeasiswaController from '@/actions/App/Http/Controllers/Admin/LaporanBeasiswaController';
...
import { Auth, Beasiswa, Periode } from '@/types';
import { Form, usePage } from '@inertiajs/react';
import { Check, ChevronDown, LoaderCircle, UploadIcon } from 'lucide-react';
import React from 'react';

export default function Create({ auth, periode, beasiswa }: { auth: Auth; periode: Periode; beasiswa: Beasiswa[] }) {
    ...
    const [beasiswaValue, setBeasiswaValue] = React.useState<string>('');
            ...
            <div className="relative flex w-full max-w-xl flex-1 flex-col overflow-x-hidden rounded bg-background shadow-xl md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2">
                <div className="px-6 py-8">
                    <div className="mb-6 flex flex-col items-center">
                        ...
                        <h2 className="text-center text-2xl font-bold text-sidebar-accent-foreground">Formulir Beasiswa Teknik Informatika</h2>
                        <h2 className="text-center text-2xl font-bold text-sidebar-accent-foreground">
                            Periode {periode.periode} {periode.tahun_mulai}
                        </h2>
                        <p className="text-center text-sm text-muted-foreground">Silakan isi data sesuai beasiswa yang telah kamu terima.</p>
                    </div>
                    {flash?.success || flash?.error ? (
                        ...
                        <Form
                            {...LaporanBeasiswaController.store.form({ id: periode.periode, tahun_mulai: periode.tahun_mulai })}
                                ...
                                <>
                                    <div className="grid grid-cols-4 gap-4 sm:grid-cols-12">
                                        ...
                                        <div className="col-span-4 grid gap-1 md:col-span-12">
                                            ...
                                            <Input type="hidden" name="beasiswa" value={beasiswaValue} required />
                                            <Popover>
                                                <PopoverTrigger asChild>
                                                    <Button
                                                        role="combobox"
                                                        variant="outline"
                                                        className={cn('w-full justify-between', !beasiswaValue && 'text-muted-foreground')}
                                                    >
                                                        {beasiswaValue
                                                            ? `${beasiswa.find((b_item) => b_item.id.toString() === beasiswaValue)?.nama_beasiswa} (Beasiswa ${beasiswa.find((b_item) => b_item.id.toString() === beasiswaValue)?.jenis_beasiswa} )`
                                                            : 'Pilih Beasiswa'}
                                                ...
                                                <PopoverContent className="w-full p-0">
                                                    <Command>
                                                        <CommandInput placeholder="Cari Beasiswa..." className="h-9" />
                                                        <CommandEmpty>Beasiswa tidak ditemukan.</CommandEmpty>

                                                        <div className="max-h-[300px] overflow-y-auto">
                                                            <CommandGroup heading="Pilih Beasiswa Yang Telah Terdaftar">
                                                                {beasiswa.map((b_item) => (
                                                                    <CommandItem
                                                                        key={b_item.id}
                                                                        value={`${b_item.nama_beasiswa} Beasiswa ${b_item.jenis_beasiswa}`}
                                                                        onSelect={() => setBeasiswaValue(b_item.id.toString())}
                                                                    >
                                                                        <Check
                                                                            className={cn(
                                                                                'mr-2 h-4 w-4',
                                                                                beasiswaValue === b_item.id.toString() ? 'opacity-100' : 'opacity-0',
                                                                            )}
                                                                        />
                                                                        {b_item.nama_beasiswa} (Beasiswa {b_item.jenis_beasiswa})
                                                                    </CommandItem>
                                                                ))}
                                                            </CommandGroup>
                                                        </div>
                                        ...
                                        {/* Jangka Waktu Penerimaan Beasiswa */}
                                        <div className="col-span-4 grid gap-1 md:col-span-12">
                                            <Label htmlFor="">Jangka Waktu Penerimaan Beasiswa</Label>
                                            ...
                                            <InputError
                                                message={errors?.penerimaan || errors?.selesai ? 'Jangka Waktu Penerimaan Beasiswa harus diisi' : ''}
                                                className="mt-2"
                                            />
                                        </div>
